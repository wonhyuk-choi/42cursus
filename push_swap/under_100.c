...
/*   Updated: 2021/06/01 20:04:22 by wonchoi          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	rotate_push_a(t_frame *frame)
{
	if (frame->small_rotate >= 0)
		...
		while (frame->big_rotate-- > 0)
			do_rb(frame);
	else if (frame->big_rrotate >= 0)
		while (frame->big_rrotate-- > 0)
			do_rrb(frame);
	do_pa(frame);
	frame->small_flag == 1 ? do_ra(frame) : 0;
	frame->big_flag == 1 ? frame->after_rotate++ : 0;
	...
	while (frame->a)
	{
		...
		while (frame->b)
		{
			...
			if (frame->b && (frame->small_rotate >= 0
					|| frame->small_rotate >= 0 || frame->big_rotate >= 0
					|| frame->big_rrotate >= 0))
